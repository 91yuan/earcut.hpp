...
namespace mapbox {
	namespace util {
		...
		template <std::size_t I, typename T> struct nth {
			inline static typename T::value_type
	...
	namespace detail {
		...
		private:
			...
			public:
				...
				T* construct(N index, double x_, double y_) {
					...
					alloc.construct(object, T(index, x_, y_));
					return object;
				}
				void reset(std::size_t newBlockSize) {
					for (std::vector<T*>::iterator itr = allocations.begin(); itr != allocations.end(); ++itr) alloc.deallocate(*itr, blockSize);
					allocations.clear();
		...
		void Earcut<N>::earcutLinked(Node* ear, int pass) {
			...
			while (ear->prev != ear->next) {
				...
				if (hashing ? isEarHashed(ear) : isEar(ear)) {
					...
					indices.push_back(prev->i);
					indices.push_back(ear->i);
		...
		template <typename N>
		struct NodeCompare {
			bool operator() (const typename Earcut<N>::Node* a, const typename Earcut<N>::Node* b) {return a->x < b->x;}
		};
		template <typename N> template <typename Polygon>
		typename Earcut<N>::Node*
			Earcut<N>::eliminateHoles(const Polygon& points, Node* outerNode) {
				...
				NodeCompare<N> myobject;
				std::sort(queue.begin(), queue.end(), myobject);
		...
		template <typename N>
		int Earcut<N>::zOrder(const double x_, const double y_) {
			// coords are transformed into non-negative 15-bit integer range
			int x = static_cast<int>(32767.0 * (x_ - minX) * inv_size);
			int y = static_cast<int>(32767.0 * (y_ - minY) * inv_size);
